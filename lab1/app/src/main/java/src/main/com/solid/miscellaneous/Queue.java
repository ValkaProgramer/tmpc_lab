...
import com.skinhead.people.Customer;
...
class Queue{
    ...
    public void dequeue(Profit profit, boolean bool, float chance, Printer printer){
    ...
    public void updateQueue(Menu menu, int season, int day, Printer printer){
        spawnEnd = new Date().getTime() / 1000;

        if(cooldown.getInterval() < spawnEnd - spawnStart){
            cooldown.setInterval(day);
            spawnStart = spawnEnd;
            this.enqueue(new Customer(menu, season), printer);
        }
    }
}